...
maximize income: sum{i in product, j in machine} (profit[i] - material_costs[i] - working_costs[j]) * production[i, j];

/*constraints*/ 
s.t. machine_available{j in machine}: sum{i in product} production[i, j] * creating_time[i, j] <= working_time;
